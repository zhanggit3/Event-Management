...
**Local `/code-review ultra` (open item, not yet changed):** the review re-flagged the **inconsistent org-scoping model** across the Company section — Clients and My Items resolve to a single org (first non-workspace), while Library › Templates aggregates *all* of the user's orgs by `created_by`. A multi-org user sees clients/files from only one org with no switcher. Creator-based management mitigates impact, but a deliberate, consistent scoping model (e.g. an org switcher shared by all three sub-pages) remains open and is deferred pending a product decision.
